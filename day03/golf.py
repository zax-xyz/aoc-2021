...
# using strings
print((y:=(lambda l,a,i=0:int(l[0],2)*int(a[0],2)if len(l+a)<3else y((b:=lambda l,c=1:[x for x in l if x[i]==str(c^(sum(1-int(y[i])*2for y in l)>0))]if len(l)>1else l)(l),b(a,0),i+1)))(l:=[*open("day3.txt")],l))
# using ints and bitwise operations (1 byte shorter)
print((y:=(lambda l,a,i=2048:l[0]*a[0]if len(l+a)<3else y((b:=lambda l,c=1:[x for x in l if(x&i>0)==c^(sum(i-(y&i)*2for y in l)>0)]if len(l)>1else l)(l),b(a,0),i>>1)))(l:=[int(x,2)for x in open("day3.txt")],l))
